...
let App = (props) => {
  ...
  let initShowMenu = (pathname) => {
    let _selectedKeys = []
    let indexList = []
    rootSubmenuKeys.forEach((item, index) => {
      let result = findKeyFunc(item, pathname, index)
      if (
        result &&
        Object.prototype.toString.call(result) === '[object Object]'
      ) {
        _selectedKeys = [result.findKey]
    ...
    deepIndex(rootSubmenuKeys, indexList)
    if (!collapsed) {
      let _openKeys = deepEachOpenKey()
  ...
  // 找到这个菜单属于哪个位置
  let findKeyFunc = (item, findKey, index, indexList = []) => {
  ...
  // 获得面包屑
  let getBreadcrumb = key => {
    let findKey = ''
    let indexList = []
    rootSubmenuKeys.forEach((item, index) => {
      let result = findKeyFunc(item, key, index)
    ...
    if (!indexList.length) {
      let oData = rootSubmenuKeys.find(el => el.path === findKey)
      if (oData) {
        setBreadcrumb([oData.menuName])
      }
    } else {
      let menuList = null
      let _breadcrumb = []
      const deep = list => {
        if (list.length) {
          menuList =
            ...
            rootSubmenuKeys[list.shift()]
            _breadcrumb.push(menuList.menuName)
          if (list.length) {
            deep(list)
          }
        }
      ...
      let lastMenuName = menuList.children.find(el => el.path === findKey)
        .menuName
        _breadcrumb.push(lastMenuName)
      setBreadcrumb(_breadcrumb)
    }
  }
  // 遍历对应的菜单栏进行,同等级的菜单关闭
  let deepEach = (list, indexList) => {
    if (indexList.length > 1) {
      let nextIndex = indexList.shift()
      deepEach(list[nextIndex].children, indexList)
    } else if (indexList.length === 1) {
  ...
  let _setOpenKeys = (key) => {
    ...
    deepEach(rootSubmenuKeys, getIndexList)
  }
  // 点击多级菜单
  let onTitleClick = ({ key }) => {
    if (!collapsed) {
      _setOpenKeys(key)
      let _openKeys = deepEachOpenKey()
      setOpenKeys(_openKeys)
    ...
    <div className={styles.admin}>
      ...
      >
        ...
        <Menu
           ...
           * 注：不需要同时打开一个，删掉openKeys={openKeys}这句话
           */
          openKeys={openKeys}
      ...
      <div
        ...
        />
        {
          <Content { ...props } breadcrumb={breadcrumb}>
            {props.children}
          </Content>
        }
        <Footer  />
